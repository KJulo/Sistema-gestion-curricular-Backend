...
async function getAlumno(id) {
  const alumno = await prisma.alumno.findUnique({ where: id, include: { curso: true } });
  if (alumno.curso){
    const asignaturas = await prisma.asignatura.findMany({ where: { curso: { id: alumno.curso.id } } })
